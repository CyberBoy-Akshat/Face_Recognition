...
def TakeImages():	
	...
	if(is_number(Id) and name.isalpha()):
		...
		harcascadePath = "D:\\FR\\haarcascade_frontalface_default_2.xml"
		...
		while(True):
			...
			for (x, y, w, h) in faces:
				...
				cv2.imwrite(
					"D:\\FR\\TrainingImage\\"+name +"."+Id +'.'+ str(
		...
		row = [Id, name]
		with open('D:\\FR\\UserDetails.csv', 'a+') as csvFile:
...
def TrainImages():
	...
	recognizer.save("D:\\FR\\Trainner.yml")
...
def TrackImages():
	...
	recognizer.read("D:\\FR\\Trainner.yml")
	harcascadePath = "D:\\FR\\haarcascade_frontalface_default_2.xml"
	faceCascade = cv2.CascadeClassifier(harcascadePath)
	# getting the name from "userdetails.csv"
	df = pd.read_csv('D:\\Pawan\\IR\\UserDetails.csv')
	cam = cv2.VideoCapture(0)
